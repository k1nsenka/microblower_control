...
int main(){
  ...
  while(EndTrigger<5){
    ...
    // 随意制御
    if((GoSign==1)&&(17<TriggerLPF[1])&&(TIMER.read()>1)){
    //&&(TriggerLPF[1]<18)){
      TRIGGER = 1;
      ASSIST = 1;
      RespiratoryAssist();
...
void RespiratoryAssist(){
  ...
  if(MODE<0.5){
    ...
    TARGET_buf = 37.5*target - 68.75;
  }
  else{
    target = TARGET*1.911 + 1.389;
    TARGET_buf = 11.249*target - 10.622;                                 // 正確には(3.3*TARGET)*(1.911/3.3)+1.389 ***
  }
